...
exports.uploadVoice = async (req, res) => {
  try {
    ...
    // Flask 서버로 음성 파일 전송
    const result = await voiceService.uploadVoiceToFlask(userId, audioFile);
...
exports.generateSpeech = async (req, res) => {
  try {
    ...
    // Flask 서버로 TTS 생성 요청
    const audioBuffer = await voiceService.generateSpeechFromFlask(userId, text);
...
exports.getVoiceStatus = async (req, res) => {
  try {
    ...
    const status = await voiceService.checkVoiceStatus(userId);
